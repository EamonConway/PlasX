...
// simulation.
PFalc::PFalc(const Parameters& params, const Status& status, double ICA,
             double ICM, double IA, double IB, double zeta)
    : current_(status),
      I_CA_(ICA),
      I_CM_(ICM),
      I_A_(IA),
...
void PFalc::scheduleInfection(const double t) { infection_queue_.emplace(t); };

bool PFalc::updateInfection(const double t) {
  // Technically, by using a priority queue there is an indirection to the
  // stack as all data is held in a vector. We could consider caching the
  // top value in the individual structure and only go away once that
  // value is outdated. If the infectionQueue is empty, the next infection
  // could be at time infinity. We would have to make sure that we update
  // things appropriately for that however. This may not be the best, at a
  // new infection we would have to check against cache value. If cahced
  // value is larger, we need to move it bak into the queue. But this
  // might just be fine. It is an interesting thing to look for.
  auto return_value = false;
  while (!infection_queue_.empty() && infection_queue_.top() <= t) {
    return_value = true;
    infection_queue_.pop();
  }
  return return_value;
...
static void SAU_infection(PFalc& state, const Parameters& params,
  ...
  // This function determines what happens with an infection in the S A or
  // U compartment. I need to determine which infections come here.
  // Infections from S A and U will go into this function, as all three
  // must go through the check of phi. You can be untreated (D) and then
  // reinfected, but you will go to D again. T and P can not be
  // re-infected, as Treatment wipes all infections that could occur, and
  // prophylaxis is killing everything off that could infect you.
  ...
  // Do not have this in the individual as we do not want accidentally
  // forget to update it.
  ...
  if (r1 <= phi) {
    // You have succeeded to go to the node that checks if you go to T or
    // D.
    if (r2 <= f_T) {
      ...
      state.update_ = [&](PFalc& state, const double lambda, const double t,
    ...
    // You failed to go to the node that checks if you go to T or D,
    // therefore you go to A.
...
// Interesting concept. We update infections, but you cant recover at the
// or do anything else. But you can possibly be infected (because it will
// go into the later step) Instead of checking if an infection occurs with
// a lagged force of infection. We will schedule the appropriate time for
// that to occur. You will either, have this schedule activate, in which
// case you can not trigger the other event, or all possible events can
// occur (infection will be schedule and the rest). However, that doesnt
// make sense, we never checked back to see if someone recovered. So....
// we shall always check if a future infection is scheduled. We then check
// if any of the scheduled events occur, and if so you can not do anything
// else. I think that this is equivalent to the way that the code used to
// be implemented.
...
static bool S_update(PFalc& state, const Parameters& params,
  ...
  // Check if an infection needs to happen this timestep. Update the
  // infection Queue - this function changes the update_ function within
  // state.
  if (state.updateInfection(t)) {
    // The individual has now got a new infection active - do the new
    // infection stuff.
    ...
    // schedule an infection. This keeps the current implementation
    // consistent with the past implementation.
    return true;
  }
...
static bool A_update(PFalc& state, const Parameters& params,
  ...
  // Reformulate to be more stable. If the top and bottom got too large
  // youd be in trouble.
  ...
  // Check and update the infection Queue - this function changes the
  // update function.
  if (state.updateInfection(t)) {
    SAU_infection(state, params, t);
...
static bool D_update(PFalc& state, const Parameters& params,
  ...
  // Check and update the infection Queue - this function changes the
  // update function. Throw away result.

  // Do we want to skip the function if this is the case?
  // We need to keep checking the queue, but we throw out any that
  // activate as we will stay in D. Hilariously, this is equivalent to
  // just being infected and going to D again thanks to the wonders of the
  // exponential distribution.
  if (state.updateInfection(t)) {
    // They go to D... so its still D update? Do nothing? Will have to
    // check.
  } else if (determine_event(prob_event, dt)) {
    // Hey something is going to happen, but what! Lets find out.
    const auto r = genunf_std(generator);  // random number
    if (r < params.r_D / prob_event) {
      ...
      state.current_ = Status::A;
      state.update_ = [&](PFalc& state, double lambda, const double t,
                          double dt) -> bool {
        return A_update(state, params, lambda, t, dt);
      };
    } else {
